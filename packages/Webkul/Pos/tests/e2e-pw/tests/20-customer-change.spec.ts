...
test("Agent able to change the cusotmer of the cart.", async ({ adminPage }) => {
    ...
    await adminPage.locator('div[title="Change Customer"]').click();

    const selectableCustomerButton = adminPage
        .locator(
            'div.flex.h-full > div:not(:has(span.icon-check)) button[title="Select Customer"]'
        )
        .first();

    await selectableCustomerButton.click();
    ...
    await adminPage.goto("pos/cashier/today-sale");
