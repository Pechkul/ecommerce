...
test("agent can also complete an order in offline mode", async ({
    ...
    await expect(adminPage.locator("div.justify-self-end").nth(0)).toBeVisible();
    ...
    await adminPage.locator("div.justify-self-end").nth(0).click();
    await adminPage.getByRole("button", { name: "Agree", exact: true }).click();
    await expect(
        adminPage.getByText("Order returned successfully.")
    ).toBeVisible();
});
