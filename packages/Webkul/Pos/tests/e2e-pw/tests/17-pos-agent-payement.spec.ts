...
import { getRandomNumber } from "../utils/myfunction";
import fs from "fs";

test("agent can also complete an order using that bank", async ({
    adminPage,
}) => {
    ...
    await adminPage.locator("#card_details").fill(getRandomNumber(4));

    await adminPage.locator("#bank_name").selectOption({
        label: JSON.parse(fs.readFileSync("bank-name.json", "utf-8")).bankname,
    });
